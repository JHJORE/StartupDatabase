...
def get_Companies(db: Session, CompanyName: str, Municipality: str, Sector: int, Type: str, EmployeesMin: int, EmployeesMax: int, skip: int = 0, limit: int = 100):
    return db.query(models.Company).filter(models.Company.Sector.like(f"%{Sector}%"), models.Company.Employees >= EmployeesMin, models.Company.Employees <= EmployeesMax, models.Company.Municipality.like(f"%{Municipality}%"), models.Aid.Type.like(f"%{Type}%"), models.Company.CompanyName.like(f"%{CompanyName}%")).offset(skip).limit(limit).all()
